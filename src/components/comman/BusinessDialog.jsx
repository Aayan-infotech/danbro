...
export const BusinessDialog = ({ open, onClose }) => {
  ...
  return (
    ...
    >
      {/* modal-header */}
      <DialogTitle id="business-dialog-title" sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", py: 1.5, px: 2, borderBottom: "1px solid", borderColor: "divider", }}>
        <CustomText sx={{ fontSize: 18, fontWeight: 600, color: "var(--themeColor)" }}>
          Business
        </CustomText>
        <IconButton onClick={onClose} aria-label="Close" sx={{ color: "text.secondary", "&:hover": { backgroundColor: "action.hover" }, }}>
          <CloseIcon />
        </IconButton>
      </DialogTitle>
